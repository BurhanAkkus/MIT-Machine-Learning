...
def perceptron_cumulative(data, labels, params={}, hook=None):
    # if T not in params, default to 100
    T = params.get('T', 100)

    # Your implementation here

    data = np.reshape(np.append(data,np.ones(np.shape(data)[1])),(np.shape(data)[0] + 1,np.shape(data)[1]))
    theta = np.zeros(np.shape(data)[0])

    for i in range(T):
        results = np.sign(labels * np.matmul(np.transpose(theta),data))
        theta = theta + np.sum(labels *  (1-results) / 2 *data ,axis= 1)
...
for datafn in (super_simple_separable_through_origin,super_simple_separable):
   data, labels = datafn()
   test_linear_classifier(datafn,perceptron,draw=False)
